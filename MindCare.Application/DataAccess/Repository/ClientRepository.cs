...
namespace MindCare.Application.DataAccess.Repository
{
    public class ClientRepository : IClientRepository
    {
        ...
        public async Task<Client> Get(int idClient)
        {
            ...
            try
            {
                _dbContext.Query = $"SELECT * FROM clients WHERE id_client={idClient}";
                await _dbContext.Connection.OpenAsync();
                _dbContext.ExecuteQuery();
                _dbContext.ExecuteReader();
        ...
        public async Task Update(Client client)
        {
            try
            {
                _dbContext.Query = "UPDATE clients " +
                    $"SET(name='{client.Name}', gender='{client.Gender}', cpf='{client.Cpf}', age={client.Age})";
                await _dbContext.Connection.OpenAsync();
                _dbContext.ExecuteQuery();
                _dbContext.ExecuteNonQuery();
        ...
        public async Task Delete(int idClient)
        {
            try
            {
                _dbContext.Query = $"DELETE FROM clients WHERE id_client={idClient}";
                await _dbContext.Connection.OpenAsync();
                _dbContext.ExecuteQuery();
                _dbContext.ExecuteNonQuery();
